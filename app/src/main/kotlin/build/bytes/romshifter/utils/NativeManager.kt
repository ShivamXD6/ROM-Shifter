    ...
    suspend fun runOperation(
        ...
        if (isBackup) {
            ...
            if (doCall) {
                ...
                writer.endArray()
                writer.close()
                Shell.cmd("su -c 'cp \"${callFile.absolutePath}\" \"$backupDir/CallLog_DB.json\"'").exec()
                callFile.delete()
            }
            if (doContacts) {
                ...
                if (vcfFile.exists()) {
                    Shell.cmd("su -c 'cp \"${vcfFile.absolutePath}\" \"$backupDir/Contacts.vcf\"'").exec()
                    vcfFile.delete()
                }
            }
        } else {
            ...
            if (doCall) {
                updateState("Restoring Call Logs...", 50)
                Shell.cmd("su -c 'appops set $pkg WRITE_CALL_LOG allow'").exec()
                val tempCall = File(context.cacheDir, "CallLog_DB.json")
                Shell.cmd("su -c 'cp \"$backupDir/CallLog_DB.json\" \"${tempCall.absolutePath}\" && chmod 666 \"${tempCall.absolutePath}\"'").exec()
                if (tempCall.exists()) {
            ...
            if (doContacts) {
                updateState("Restoring Contacts (vCard)...", 90)
                val tempVcf = File(context.cacheDir, "Contacts.vcf")
                Shell.cmd("su -c 'cp \"$backupDir/Contacts.vcf\" \"${tempVcf.absolutePath}\" && chmod 666 \"${tempVcf.absolutePath}\"'").exec()
                if (tempVcf.exists()) {
